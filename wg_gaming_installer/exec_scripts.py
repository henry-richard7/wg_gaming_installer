...
def create_wg_config(
    ...
    with open(wg_conf_path, 'w') as f:
        ...
        f.write("[Interface]\n")
        if wg_config.ipv6:
            f.write(f"Address = {wg_config.ipv4!s}, {wg_config.ipv6!s}\n")
        else:
        ...
        f.write(f'PostDown = "{stop_script_path!s}"\n')
        ...
        for peer in peer_config:
            ...
            if peer.ipv6:
                f.write(f"AllowedIPs = {peer.ipv4.ip!s}/32, {peer.ipv6.ip!s}/128\n")
            else:
                f.write(f"AllowedIPs = {peer.ipv4.ip!s}/32\n")
            f.write("\n")
...
def create_start_script(
    ...
    with open(start_script_path, 'w') as f:
        # Append shebang line
        _append_shebang(f, shell_path)

        # Append sysctl forwarding commands
        _append_forwarding(f, ipv6_as_well=has_ipv6)
        ...
        f.write(f"nft -f {nftables_conf_path.resolve()!s}\n")
...
def create_stop_script(
    ...
    with open(stop_script_path, 'w') as f:
        ...
        _append_disable_forwarding(f, ipv6_as_well=has_ipv6)
...
def _append_shebang(file: TextIO, shell_path: Path) -> None:
    """
...
def _append_forwarding(file: TextIO, ipv6_as_well: bool) -> None:
    """
...
def _append_disable_forwarding(file: TextIO, ipv6_as_well: bool) -> None:
    """
